...
import { activatePlatformUser, invitePlatformUser, listPlatformUsers, suspendPlatformUser } from '../../services/platformUserAdminService'
import type { PlatformOrganization } from '../../types/platformAdmin'
import type { PlatformAdminUser } from '../../types/platformUserAdmin'
import { invitationStatusLabels, validatePlatformUserEmail, validatePlatformUserName, validatePrimaryOrganization, validateSuspensionReason } from './platformUsersModel'
...
export function PlatformUsersSection({ organizations }: Props) {
  ...
  return (
    <section className="platform-modules-panel platform-users-panel" aria-labelledby="platform-users-title">
      <div className="platform-modules-header"><div><span className="eyebrow">المستخدمون والدعوات</span><h2 id="platform-users-title">إدارة المستخدمين</h2><p>دعوة المستخدمين ومتابعة حالتهم وإيقاف الحساب أو تفعيله دون حذف الأعمال أو الإسنادات.</p></div><div className="platform-user-header-actions"><button className="secondary-button" type="button" onClick={() => void load()} disabled={loading || busyId !== null}>تحديث القائمة</button><button className="primary-button" type="button" onClick={openInvite} disabled={busyId !== null || activeOrganizations.length === 0}><Icon name="users" size={18} /> دعوة مستخدم</button></div></div>
      {activeOrganizations.length === 0 ? <div className="platform-modules-feedback error" role="status">لا يمكن إرسال دعوة قبل إنشاء جهة فعالة. لم تُنشأ أي بيانات تلقائيًا.</div> : null}
