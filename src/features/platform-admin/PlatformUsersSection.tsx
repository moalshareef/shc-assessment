...
import { activatePlatformUser, createPlatformUser, invitePlatformUser, listPlatformUsers, resetPlatformUserPassword, suspendPlatformUser } from '../../services/platformUserAdminService'
...
export function PlatformUsersSection({ organizations }: Props) {
  ...
  const [resetTarget, setResetTarget] = useState<PlatformAdminUser | null>(null)
  ...
  async function submitPasswordReset(event: FormEvent) {
    ...
    if (passwordError) nextErrors.temporaryPassword = passwordError
    if (confirmTemporaryPassword !== temporaryPassword) nextErrors.confirmTemporaryPassword = 'تأكيد كلمة المرور غير مطابق.'
    setErrors(nextErrors)
    if (Object.keys(nextErrors).length) return
    if (!window.confirm(`هل تريد تعيين كلمة مرور مؤقتة جديدة للمستخدم «${resetTarget.fullName || resetTarget.email}»؟`)) return

    setBusyId(`reset:${resetTarget.userId}`)
    setFeedback(null)
    try {
      await resetPlatformUserPassword({ userId: resetTarget.userId, temporaryPassword })
      closePasswordReset()
      setFeedback({ type: 'success', message: 'تم تعيين كلمة مرور مؤقتة، وسيُطلب من المستخدم تغييرها عند الدخول.' })
      await load()
    } catch (error) {
      setTemporaryPassword('')
      setConfirmTemporaryPassword('')
      setFeedback({ type: 'error', message: error instanceof Error ? error.message : 'تعذر إعادة تعيين كلمة المرور.' })
    } finally {
      setBusyId(null)
    }
  }
