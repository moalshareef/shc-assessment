...
### 1.2 فحص أدوار المنصة — 15 يوليو 2026

- `profiles.role` مقيد بـ`system_admin/workspace_owner/manager/member/viewer` ولا يحتوي أدوار المنصة الجديدة.
- يوجد Profile نشط واحد بدور `member`، ولا يوجد `system_admin` حاليًا.
- المستخدم الحالي يملك عضويتين تشغيليتين `workspace_members.owner` للرقابة المالية والركائز؛ لا تتحولان تلقائيًا إلى `system_owner` أو `platform_admin`.
- `auth.users.raw_app_meta_data` للمستخدم الحالي يحتوي Provider البريد فقط، و`raw_user_meta_data` يحتوي تأكيد البريد؛ لا توجد مطالبة دور منصي موثوقة، ولا يستخدم Metadata لاتخاذ قرار الصلاحية.
- `platform_modules` و`organizations` موجودان وفارغان، ولا يوجد جدول `platform_role_assignments` قبل هذه المسودة.
- لا تعارض بنيوي مع الجدول الجديد لأن الهوية تعاد من `profiles.id` والتدقيق يعاد من `audit_logs`، بينما تبقى أدوار Workspace مستقلة وتشغيلية.
...
| `platform_role` | `text` | `system_owner/platform_admin/auditor/viewer` |
| `status` | `text` | `scheduled/active/expired/revoked` |
| `starts_at` | `timestamptz` | nullable؛ `NULL` يعني البدء من `created_at` |
| `ends_at` | `timestamptz` | nullable، أكبر من البداية |
| `revoked_at/revoked_reason` | وقت/نص | مطلوبان مع `revoked` فقط |
...
**القيود المعتمدة:** لا حذف فعلي، ولا إعادة تفعيل لسجل ملغى، ومنع النوافذ الزمنية المتداخلة للمستخدم والدور نفسيهما عبر Trigger وقفل Advisory ذري. يوجد `system_owner` فعال واحد فقط في أي لحظة ولا يسمح له بتاريخ انتهاء. لا ينشأ الحساب أو الدور في هذه المرحلة.

**تسليم الملكية:** لأن شرط «مالك واحد» يتعارض منطقيًا مع اشتراط وجود مالكين نشطين في اللحظة نفسها، اعتمد تسليم ذري: RPC الإلغاء يقفل السجل، يلغي المالك الحالي، وينشئ للـUUID البديل النشط دور `system_owner` داخل المعاملة نفسها. لا توجد لحظة مرئية بلا مالك ولا حالة مستقرة بمالكين فعالين.

**قرار الصلاحية:** يعتمد على Profile نشط، وسجل غير `revoked`، وبداية نافذة وصلت ونهاية لم تصل. لا يعتمد على `status` المخزن وحده؛ `platform_refresh_role_status` يزامن الحالة للتقارير فقط.
...
#### التنفيذ الانتقالي المعتمد للمرحلة الأولى

لأن `platform_modules` لا يحتوي سجلات بعد، بينما `workspaces` يحتوي المساحات التشغيلية الثلاث، ينفذ الجدول في هذه المرحلة مع `workspace_id` إلزامي و`module_id` اختياري. يملأ `module_id` تلقائيًا متى وجد الربط الواحد إلى واحد مع الـWorkspace، دون إنشاء موديل أو تعديل Workspace. هذا يمنع بناء جدول بديل ثم ترحيله لاحقًا.

الأدوار المقيدة حاليًا هي `financial_control_employee` و`financial_control_manager` فقط، والنطاقات هي `assigned_records/organization_records/all_records`. السجل الفعال أو المجدول واحد لكل `(user_id, workspace_id, organization_id)`، والإلغاء تاريخي بلا حذف. تبقى `workspace_members` و`financial_control_members` مصادر توافق مؤقتة للعضويات الحالية، ويقرأ قرار RLS المصدر المركزي معها دون تعديل صفوفها.
